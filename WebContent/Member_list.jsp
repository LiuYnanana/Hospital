	...
	<div class="x-body">
		<div class="layui-row">
			<form class="layui-form layui-col-md12 x-so" action="MemberFind">
				<input class="layui-input" placeholder="请输入科室名称" name="departmentId"
					id="input1">
		...
		<xblock> <a href="member-add.jsp"><button
				class="layui-btn">
			...
			<tbody>
				<%
	    		 ...
	    		 <tr>
					...
					<td class="td-manage">
						<button class="layui-btn layui-btn-sm" onclick="deleteTip('<%=dp.getDepartmentName()%>')">删除</button> 
						<a href="member-edit.jsp?editName=<%=dp.getDepartmentName()%>">
		...
		<div class="page">
			<div>
              ...
              %>
				...
				<a class="prev" href="ChangePage?page=<%=Page - 1%>&&pos=Member_list.jsp">&lt;&lt;</a> 
				<a class="num" href="ChangePage?page=1&&pos=Member_list.jsp">首页</a>
				  <strong>第<%out.print(Page);%>页/共<%out.print(Pagenumber);%>页</strong>
				<a class="num" href="ChangePage?page=<%=Pagenumber%>&&pos=Member_list.jsp">尾页</a> 
			    <a class="next" href="ChangePage?page=<%=Pagenumber%>&&pos=Member_list.jsp">&gt;&gt;</a>
			</div>
		</div>
	</div>
</div>
...
<script>
	$(document).ready(function(){
	  $("#btn1").click(function(){
		  ...
		  $.ajax({
	          url:"<%=request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()+ request.getContextPath() + "/"%>MemberFind",//访问的地址
	          type:"post",
	          data:{
	          ...
	          success:function(data){
	        	  if(data == "yes") {
	        		  window.location.href="<%=request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()+ request.getContextPath() + "/"%>member-find.jsp";
	        	  }
	        	  else {
	        		  $("#msg").html("无此科室信息！");
                      $('#tip').modal('show');
 	...
 	function deleteTip(name) {
 		if(confirm("确定删除吗？")) {
			  $.ajax({
		          url:"<%=request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()+ request.getContextPath() + "/"%>MemberDel",//访问的地址
		          type:"post",
		          data:{
		        	   "dpName" : name,
		        	  },
